...
const message =
    ...
    `🕒 <b>Sana:</b> ${orderDate}\n\n` +
    `<b>📦 Mahsulotlar:</b>\n${itemsList}\n\n` +
    `${Number(data.productDiscountAmount) > 0 ? `💵 <b>Asl summa:</b> ${(Number(data.originalTotal) || 0).toLocaleString()} so'm\n` : ""}` +
    `${Number(data.productDiscountAmount) > 0 ? `🏷 <b>Mahsulot chegirmasi:</b> -${Number(data.productDiscountAmount).toLocaleString()} so'm\n` : ""}` +
    `${Number(data.couponDiscountAmount) > 0 ? `🎟 <b>Kupon chegirmasi:</b> -${Number(data.couponDiscountAmount).toLocaleString()} so'm\n` : ""}` +
    ...
    `<i>Tez orada operatorimiz siz bilan bog‘lanadi!</i>`;
        ...
        await bot.telegram.sendMessage(ADMIN_TELEGRAM_ID, message, { parse_mode: 'HTML' });
